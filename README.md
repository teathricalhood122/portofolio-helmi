# Hi there, I'm Helmi! 👋

### 🚀 Business Information Systems Student | Fullstack Laravel & Automation Enthusiast

Welcome to my personal portfolio repository! This project showcases my journey as a developer, highlighting my skills in web development and workflow automation.

🌐 **Live Demo:** [https://portofolio-helmi-ecru.vercel.app/](https://portofolio-helmi-ecru.vercel.app/) 

---

### 🛠️ Tech Stack Used
* **Frontend:** HTML5, CSS3 (Custom High Contrast), Bootstrap 5
* **Backend Experience:** Laravel 10, PHP, MySQL (Applied in projects)
* **Automation:** n8n, Telegram Bot API, Google Sheets Integration
* **Tools:** VS Code, Git, GitHub, Vercel

---

### 📂 Featured Projects
1.  **Medika Mulia** - Clinic Management System with RBAC.
2.  **Auto Receipt Tracker** - Automated OCR workflow using n8n.
3.  **Herbigreen CMS** - Content management and digital branding.

---

### 📫 Let's Connect!
* **LinkedIn:** [linkedin.com/in/helmifajari]( https://id.linkedin.com/in/helmi-mohammad-fajari-74b8422a7)
* **Email:** (helmifajari1927@gmail.com)

_Thanks for visiting! Don't forget to star ⭐ this repo if you find it cool._
